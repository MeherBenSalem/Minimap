...
public class MinimapHudRenderer {
    ...
    public static void render(GuiGraphicsExtractor graphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        if (!ClientEvents.isMinimapVisible()) return;
        if (mc.player == null || mc.level == null) return;
        if (mc.gui.screen() instanceof FullscreenMapScreen) return;
        if (OdysseyConfig.HIDE_WHEN_SCREEN_OPEN.get() && mc.gui.screen() != null) return;
        ...
        if (tex != null) {
            graphics.blit(RenderPipelines.GUI_TEXTURED, tex, 0, 0, 0, 0, size, size, size, size, tint);
        }
        ...
        if (OdysseyConfig.SHOW_COORDINATES.get()) {
            ...
            graphics.text(mc.font, coords, x, y + size + 4, 0xFFFFFF, true);
        }
    }

    public static void drawBorder(GuiGraphicsExtractor graphics, int x, int y, int size) {
    ...
    public static void drawCompass(GuiGraphicsExtractor graphics, int x, int y, int size, float yaw) {
        Minecraft mc = Minecraft.getInstance();
        int cx = x + size / 2;
        int cy = y + size / 2;
        int r = size / 2 + 6;
        ...
        for (int i = 0; i < 4; i++) {
            ...
            graphics.centeredText(mc.font, labels[i], lx, ly - 4, 0xFFDDDDDD);
        }
    }
}
