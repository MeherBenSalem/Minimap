...
public class FullscreenMapScreen extends Screen {
    ...
    private void createWaypoint() {
        ...
        minecraft.gui.setScreen(WaypointEditScreen.forCreate(
    ...
    private void editWaypoint(Marker marker) {
        if (minecraft != null) {
            minecraft.gui.setScreen(WaypointEditScreen.forEdit(this, marker));
        }
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        // Map frame — 1px border
        if (mapPanel != null) {
            int px = mapPanel.getX();
            int py = mapPanel.getY();
            int pw = mapPanel.getWidth();
            int ph = mapPanel.getHeight();
            graphics.fill(px - 1, py - 1, px + pw + 1, py, 0xFF505060);       // top
            graphics.fill(px - 1, py + ph, px + pw + 1, py + ph + 1, 0xFF505060); // bottom
            graphics.fill(px - 1, py, px, py + ph, 0xFF505060);               // left
            graphics.fill(px + pw, py, px + pw + 1, py + ph, 0xFF505060);     // right
        }

        // Sidebar background
        graphics.fill(width - SIDEBAR_W, TOP_BAR_H, width, height, 0xE0181820);
        graphics.fill(width - SIDEBAR_W, TOP_BAR_H, width - SIDEBAR_W + 1, height, 0xFF404050);

        super.extractRenderState(graphics, mouseX, mouseY, partialTick);

        // Coordinates overlay — gated by config
        if (OdysseyConfig.MAP_SHOW_COORDINATES.get()) {
            int worldX = screenToWorldX(mouseX);
            int worldZ = screenToWorldZ(mouseY);
            if (mapPanel != null) {
                graphics.text(font, "X: " + worldX + "  Z: " + worldZ,
                        mapPanel.getX(), mapPanel.getY() + mapPanel.getHeight() - 10, 0xFFCCCCCC, true);
            }
        }

        // Title + dimension
        graphics.centeredText(font, title, width / 2 - SIDEBAR_W / 2, 6, 0xFFFFFFFF);
        if (minecraft != null && minecraft.level != null) {
            String dim = DimensionUtil.dimensionId(minecraft.level.dimension());
            graphics.text(font, dim, 4, height - 12, 0xFF888899, false);
        }

        // Zoom level display — between recenter button and zoom buttons
        String zoomText = "Z:" + zoom;
        graphics.text(font, zoomText, width - SIDEBAR_W - 96, 6, 0xFFAAAAAA, false);

        // Sidebar label is already indicated by the search box placeholder
    }
