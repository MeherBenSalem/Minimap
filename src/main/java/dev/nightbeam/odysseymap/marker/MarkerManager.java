...
public class MarkerManager {
    ...
    private void updatePlayerMarkers(Minecraft mc) {
        ...
        if (!OdysseyConfig.SHOW_PLAYERS.get()) {
            return;
        }
    ...
    private void updateEntityMarkers(Minecraft mc) {
        ...
        for (Entity entity : level.entitiesForRendering()) {
            if (entity == self || entity instanceof AbstractClientPlayer) {
                continue;
            }
            if (self.distanceTo(entity) > 64) {
                continue;
            }
    ...
    public void setDeathPoint(ResourceKey<Level> dimension, int x, int z) {
        if (!OdysseyConfig.SHOW_DEATH.get()) {
            return;
        }
    ...
    public void setBedPoint(ResourceKey<Level> dimension, int x, int z) {
        if (!OdysseyConfig.SHOW_BEDS.get()) {
            return;
        }
    ...
    public void addWaypointDirect(Marker marker) {
        waypoints.add(marker);
    }
