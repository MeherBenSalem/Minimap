...
public final class MarkerRenderer {
    ...
    public static void renderHud(GuiGraphics graphics, Minecraft mc, int mapX, int mapY, int size,
        ...
        for (Marker marker : markers) {
            ...
            if (OdysseyConfig.SHOW_MARKER_DISTANCE.get()) {
                ...
                graphics.drawCenteredString(mc.font, text, px, py - 10, 0xFFFFFF);
            }
        }
        ...
        if (headingUp) {
            graphics.pose().mulPose(com.mojang.math.Axis.ZP.rotationDegrees(0));
        } else {
    ...
    public static void renderFullscreen(GuiGraphics graphics, Minecraft mc, int mapX, int mapY, int mapW, int mapH,
        ...
        for (Marker marker : markers) {
            if (marker.getType() == MarkerType.PLAYER) {
                continue;
            }
            ...
            int px = cx + (int) dx;
            int py = cy + (int) dz;
            if (px < mapX || py < mapY || px > mapX + mapW || py > mapY + mapH) {
                continue;
            }
