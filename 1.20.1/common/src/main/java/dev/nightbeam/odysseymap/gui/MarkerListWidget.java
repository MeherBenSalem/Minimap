...
public class MarkerListWidget extends AbstractWidget {
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = 0; i < visible && i + scroll < filtered.size(); i++) {
            ...
            graphics.drawString(mc.font, star, rowX, rowY + 3, starColor, false);
            rowX += STAR_W;

            String eye = m.isVisible() ? "\u25C9" : "\u25CE";
            int eyeColor = m.isVisible() ? 0xFF88CC88 : 0xFF555555;
            graphics.drawString(mc.font, eye, rowX, rowY + 3, eyeColor, false);
            rowX += EYE_W;
            ...
            graphics.drawString(mc.font, label, rowX, rowY + 3, labelColor, false);
        }
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        else if (delta < 0) scroll = Math.min(Math.max(0, filtered.size() - 1), scroll + 1);
        return true;
    }
